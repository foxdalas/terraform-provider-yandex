...
- `replication_runtime` (Block List) Replication runtime parameters for the transfer (see [below for nested schema](#nestedblock--replication_runtime))
- `runtime` (Block List) Runtime parameters for the transfer (see [below for nested schema](#nestedblock--runtime))
...
- `transfer_id` (String) Identifier of the transfer to be returned.
 ...
 To get the list of all available transfers, make a [List] request.
- `transformation` (Block List) Transformation for the transfer. (see [below for nested schema](#nestedblock--transformation))
...
- `upload_shard_params` (Block List) Parallel snapshot parameters (see [below for nested schema](#nestedblock--replication_runtime--yc_runtime--upload_shard_params))
...
- `job_count` (Number) Number of workers in parallel replication.
- `upload_shard_params` (Block List) Parallel snapshot parameters (see [below for nested schema](#nestedblock--runtime--yc_runtime--upload_shard_params))
...
- `transformers` (Block List) A list of transformers. You can specify exactly 1 transformer in each element of
 list
...
- `convert_to_string` (Block List) Convert column values to strings (see [below for nested schema](#nestedblock--transformation--transformers--convert_to_string))
- `filter_columns` (Block List) Set up a list of table columns to transfer (see [below for nested schema](#nestedblock--transformation--transformers--filter_columns))
- `filter_rows` (Block List) This filter only applies to transfers with queues (Logbroker or Apache Kafka®)
 as a data source. When running a transfer, only the strings meeting the
 specified criteria remain in a changefeed. (see [below for nested schema](#nestedblock--transformation--transformers--filter_rows))
- `mask_field` (Block List) Mask field transformer allows you to hash data (see [below for nested schema](#nestedblock--transformation--transformers--mask_field))
- `rename_tables` (Block List) Set rules for renaming tables by specifying the current names of the tables in
 the source and new names for these tables in the target (see [below for nested schema](#nestedblock--transformation--transformers--rename_tables))
- `replace_primary_key` (Block List) Override primary keys. (see [below for nested schema](#nestedblock--transformation--transformers--replace_primary_key))
- `sharder_transformer` (Block List) Set the number of shards for particular tables and a list of columns whose
 values will be used for calculating a hash to determine a shard. (see [below for nested schema](#nestedblock--transformation--transformers--sharder_transformer))
- `table_splitter_transformer` (Block List) Splits the X table into multiple tables (X_1, X_2, ..., X_n) based on data. (see [below for nested schema](#nestedblock--transformation--transformers--table_splitter_transformer))
